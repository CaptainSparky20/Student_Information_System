...
def unified_dashboard(request):
    ...
    if user.role == CustomUser.Role.ADMIN:
        context.update(
            {
                "total_lecturers": CustomUser.objects.filter(
    ...
    # ---------------- Lecturer ----------------
        ...
        lecturer = (
            CoreLecturer.objects.filter(user=user).select_related("department").first()
        )
        if not lecturer:
        ...
        enrollments = Enrollment.objects.filter(
            class_group__in=classgroups
        ).select_related("student__user", "class_group")

        today = date.today()
        todays_attendance_count = (
            Attendance.objects.filter(enrollment__in=enrollments, date=today)
        ...
        try:
            from notifications.models import (
                Notification,
            )  # adjust or remove if you don’t have it
    ...
    # ---------------- Student ----------------
        ...
        try:
            student = CoreStudent.objects.select_related("class_group__course").get(
                user=user
            )
        ...
        achievements_qs = StudentAchievement.objects.filter(student=student).order_by(
            "-date_awarded"
        )
        disciplinary_qs = DisciplinaryAction.objects.filter(student=student).order_by(
            "-date"
        )
...
def profile_view(request):
    ...
    if user.role == user.Role.STUDENT:
        ...
        try:
            lecturer = Lecturer.objects.select_related("user", "department").get(
                user=user
            )
...
def parent_update(request, pk):
    ...
    return render(
        request,
        "dashboard/parent_profile_update.html",

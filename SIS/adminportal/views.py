...
from .forms import (
    LecturerCreationForm,
    StudentUpdateForm,
    StudentProfileUpdateForm,
    CourseForm,
    DepartmentForm,
    AddStudentForm,
    AssignLecturersToClassGroupForm,
    ClassGroupForm,
    SubjectForm,
)
...
def lecturer_list(request):
    ...
    lecturers = Lecturer.objects.select_related("user", "department").prefetch_related(
        "subjects", "classgroups"
    )
    ...
    return render(
        request,
        "adminportal/lecturer_part/lecturer_list.html",
        {
            "lecturers": lecturers,
...
def add_lecturer(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save_m2m()
            messages.success(
                request, "Lecturer added successfully.", extra_tags="lecturer"
            )
            return redirect("adminportal:lecturer_list")
        else:
            messages.error(
                request,
                "Please correct the errors in the lecturer form.",
                extra_tags="lecturer",
            )
    else:
    ...
    return render(
        request,
        "adminportal/lecturer_part/add_lecturer.html",
        {
            "lecturer_form": form,
            "departments": Department.objects.all(),
...
def export_lecturers(request):
    lecturers = Lecturer.objects.select_related("user__department").prefetch_related(
        "classgroups"
    )
...
def student_list(request):
    ...
    if classgroup_id:
        students = students.filter(
            student__enrollment__class_group_id=classgroup_id
        ).distinct()
    ...
    return render(
        request,
        "adminportal/student_part/student_list.html",
        {
            "students": students,
            "departments": departments,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if CustomUser.objects.filter(email=data["email"]).exists():
                ...
                form.add_error(
                    "identity_card_number", "A user with this IC number already exists."
                )
            else:
                # Create the user
                ...
                Enrollment.objects.create(
                    student=student, class_group=data["class_group"]
                )

                messages.success(
                    request,
                    f"Student {user.full_name} added and enrolled successfully.",
                )
...
def student_detail(request, pk):
    ...
    return render(
        request,
        "adminportal/student_part/student_detail.html",
        {
            "user": user,
...
def enroll_student(request, pk):
    ...
    if request.method == "POST":
        ...
        if Enrollment.objects.filter(student=student, class_group=classgroup).exists():
            messages.warning(
                request,
                f"{student.user.full_name} is already enrolled in {classgroup.name}.",
            )
        else:
            Enrollment.objects.create(student=student, class_group=classgroup)
            messages.success(
                request,
                f"{student.user.full_name} enrolled in {classgroup.name} successfully.",
            )
        return redirect("adminportal:student_list")

    return render(
        request,
        "adminportal/student_part/enroll_student.html",
        {
            "student": student,
            "classgroups": classgroups,
        },
...
def export_students(request):
    students = CustomUser.objects.filter(role=CustomUser.Role.STUDENT).select_related(
        "department"
    )
    response = HttpResponse(content_type="text/csv")
...
def staff_list(request):
    staff_users = CustomUser.objects.filter(role=CustomUser.Role.ADMIN).order_by(
        "full_name", "short_name"
    )
    return render(
        request, "adminportal/staff_part/staff_list.html", {"staff_users": staff_users}
    )
...
def add_staff(request):
    if not request.user.is_superuser:
        messages.error(
            request,
            "You are not permitted to add staff. Only superusers can perform this action.",
            extra_tags="staff",
        )
        return render(
            request,
            "adminportal/staff_part/add_staff.html",
            {
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Please correct the errors in the staff form.",
                extra_tags="staff",
            )
    else:
...
def course_list(request):
    ...
    return render(
        request, "adminportal/course_part/course_list.html", {"courses": courses}
    )
...
def edit_course(request, pk):
    ...
    return render(
        request,
        "adminportal/course_part/edit_course.html",
...
def assign_lecturer_classgroup(request, pk):
    ...
    return render(
        request,
        "adminportal/course_part/assign_lecturer_course.html",
        {
            "form": form,
...
def export_courses(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="courses_{slugify(str(request.user))}.csv"'
    )
    ...
    for course in Course.objects.all():
        ...
        subjects_str = (
            ", ".join([f"{s.name} ({s.code})" for s in subjects]) if subjects else "-"
        )
        ...
        if classgroups:
            for idx, cg in enumerate(classgroups):
                ...
                lecturer_names = (
                    ", ".join([l.user.full_name for l in lecturers])
                    if lecturers
                    else "-"
                )
                lecturer_emails = (
                    ", ".join([l.user.email for l in lecturers]) if lecturers else "-"
                )
                ...
                writer.writerow(
                    [
                        course.name
                        if idx == 0
                        else "",  # Only show course name on first group
                        course.code
                        if idx == 0
                        else "",  # Only show code on first group
...
def department_list(request):
    ...
    return render(
        request,
        "adminportal/department_part/department_list.html",
        {"departments": departments},
    )
...
def add_department(request):
    if request.method == "POST":
        ...
        form = DepartmentForm()
    return render(
        request, "adminportal/department_part/add_department.html", {"form": form}
    )
...
def add_classgroup(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Class group '{classgroup.name}' added successfully."
            )
            return redirect("adminportal:edit_course", pk=classgroup.course.pk)
    else:
        form = ClassGroupForm(initial=initial)
    return render(
        request, "adminportal/course_part/add_classgroup.html", {"form": form}
    )
...
def get_classgroups_by_department(request):
    ...
    html = render_to_string(
        "adminportal/student_part/_classgroup_dropdown.html", {"form": form}
    )
...
def edit_classgroup(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Class group '{classgroup.name}' updated successfully."
            )
            return redirect("adminportal:edit_course", pk=classgroup.course.pk)
    else:
    ...
    return render(
        request,
        "adminportal/course_part/edit_classgroup.html",
        {
            "form": form,
            "classgroup": classgroup,
            "course": classgroup.course,
...
def fee_plan_list(request):
    ...
    plans = (
        StudentFeePlan.objects.select_related("student__user")
...
def fee_plan_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            plan = form.save()
            messages.success(
                request, "Fee plan created. You can now generate installments."
            )
    ...
    return render(
        request,
        "adminportal/student_part/fees/fee_plan_form.html",
        {"form": form, "creating_for_student": None},
    )
...
def fee_plan_create_for_student(request, student_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            plan.save()
            messages.success(
                request, f"Fee plan created for {student.user.get_full_name()}."
            )
            return redirect("adminportal:fee_plan_detail", plan_id=plan.id)
    else:
    ...
    return render(
        request,
        "adminportal/student_part/fees/fee_plan_form.html",
        {"form": form, "creating_for_student": student},
    )
...
def fee_plan_detail(request, plan_id):
    plan = get_object_or_404(
        StudentFeePlan.objects.select_related("student__user").prefetch_related(
            "installments"
        ),
        pk=plan_id,
    )
    ...
    return render(
        request, "adminportal/student_part/fees/fee_plan_detail.html", context
    )
...
def installment_toggle_paid(request, pk):
    inst = get_object_or_404(
        StudentFeeInstallment.objects.select_related("plan"), pk=pk
    )
    ...
    messages.success(
        request,
        f"Installment #{inst.sequence_no} is now {'PAID' if inst.is_paid else 'UNPAID'}.",
    )
